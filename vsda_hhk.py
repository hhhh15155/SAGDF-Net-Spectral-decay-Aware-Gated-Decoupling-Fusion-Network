# train_vsda_hhk.py - VSDANet training for HHK dataset
...
SAVE_DIR_PARAMS = 'cls_params/hhk'
SAVE_DIR_RESULTS = 'cls_result/hhk'
SAVE_DIR_MAPS = 'cls_map/hhk'
...
def loadData():
    """Load HHK dataset (8chengliu)"""
    data_HSI = sio.loadmat('data/8chengliu/8chengliu_HSI.mat')['data']
    data_lidar = sio.loadmat('data/8chengliu/8chengliu_LiDAR.mat')['data']
    labels = sio.loadmat('data/8chengliu/label.mat')['label']
...
def acc_reports(y_test, y_pred_test):
    ...
    target_names = [
        'Shriveled spartina alterniflora',
    ...
    classification = classification_report(y_test, y_pred_test, digits=4,
                                           target_names=target_names, labels=range(6))
...
def main():
    print('=' * 70)
    print('VSDANet Training on HHK Dataset')
    print('=' * 70)
    ...
    for epoch in pbar:
        ...
        if (epoch + 1) % 20 == 0:
            ...
            if test_acc > best_acc:
                best_acc = test_acc
                best_epoch = epoch + 1
                torch.save(model.state_dict(), f'{SAVE_DIR_PARAMS}/VSDANet_HHK_best.pth')
    ...
    print('\n--- Final Testing ---')
    model.load_state_dict(torch.load(f'{SAVE_DIR_PARAMS}/VSDANet_HHK_best.pth'))
    ...
    # Save results
    with open(f'{SAVE_DIR_RESULTS}/VSDANet_HHK_report.txt', 'w') as f:
        f.write('=' * 70 + '\n')
        f.write('VSDANet: HHK Dataset (8chengliu, 6 classes)\n')
        f.write('=' * 70 + '\n\n')
    ...
    with open(f'{SAVE_DIR_RESULTS}/VSDANet_HHK_history.json', 'w') as f:
    ...
    if GENERATE_LABELED_MAP and total_iter is not None:
        ...
        Utils.generate_png(
            ...
            f'{SAVE_DIR_MAPS}/VSDANet_HHK_classification'
        )
